...
describe('UX-GO-001 CTA-001 Option A: card-required trial copy', () => {
  test('default (no promo): byte-exact card-honest line, 14-day fail-quiet base', () => {
    const d = getTrialDisplay(null, 'none', null)
    expect(d.days).toBe(GO_BASE_TRIAL_DAYS)
    expect(d.subtext).toBe(
      "Card required — you won't be charged before your 14-day free trial ends. Cancel anytime."
    )
  })
  ...
  test('valid longer promo: days flow from the promo path (never hardcoded 14)', () => {
    ...
    expect(d.subtext).toBe(
      "Card required — you won't be charged before your 30-day free trial ends. Cancel anytime."
    )
  })
  ...
  test('the trial-days number is NOT hardcoded in the page or the copy template', () => {
    ...
    expect(landingSrc).toContain("Card required — you won't be charged before your ${days}-day free trial ends. Cancel anytime.")
  })
...
describe('UX-GO-001 CTA-002: completion state copy (byte-exact)', () => {
  ...
  test('completion copy: Marc-approved heading + button, footnote REMOVED (CTA-004)', () => {
    expect(GO_CTA_COPY_COMPLETED.heading).toBe("That's where Episode 1 ends — for now.")
