...
export default function HorizontalStoryCard({ id, title, genre, author, duration_mins, cover_url, description, credits, series_number, series_total, flags = [], flag, progress_percent, avg_rating, review_count, is_completed, has_reviewed, not_for_me, onReviewClick }: HorizontalStoryCardProps) {
  ...
  return (
    <Link href={`/player/${id}?autoplay=1&playNow=1`} style={{ display: 'flex', background: '#1e293b', borderRadius: '14px', position: 'relative', overflow: 'hidden', border: '1px solid rgba(148,163,184,0.06)', textDecoration: 'none', alignItems: 'stretch', padding: 0, minHeight: '100px' }}>
      <div style={{ flexShrink: 0, border: '10px solid #1e293b', borderRight: 'none', display: 'flex', alignItems: 'center' }}>
        <div style={{ width: '80px', height: '80px', borderRadius: '6px', overflow: 'hidden', flexShrink: 0, boxShadow: '0 0 15px rgba(255,255,255,0.4)', position: 'relative' }}>
          <img src={cover_url || '/images/default-cover.png'} alt={title} style={{ width: '80px', height: '80px', objectFit: 'cover', display: 'block' }} />
      ...
      <div style={{ flex: 1, padding: '10px 12px 10px 10px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
        ...
        <h3 style={{ color: 'white', fontSize: '13px', fontWeight: 700, margin: 0, lineHeight: 1.2, letterSpacing: '-0.01em', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{title}</h3>
