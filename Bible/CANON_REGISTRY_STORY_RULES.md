...
## LOUDNESS-001
Status: ACTIVE | Scope: All voiced narrative and character dialogue segments in core.ts assembly; BEAT/pause segments, Belle audio, sting/transition audio, and music bed are excluded via getSpokenSegmentNumbers and are not audited | Last Revised: Sep 7, 2026
Purpose: Detect near-silent or unmeasurable voiced segments before final mix and block assembly, requiring re-render. Prevents corrupted or silent ElevenLabs renders from surviving into the finished episode undetected.
Rule: During assembly (core.ts), every voiced segment (narrator and character dialogue, identified by getSpokenSegmentNumbers) is measured for integrated LUFS. Any segment with a successfully measured LUFS below −28 is classified as near-silent or corrupted and causes assembly to hard-fail before the mix — flagged for re-render. Segments that cannot be measured (analyzeAudioLoudness throws or returns non-finite) are logged and skipped pending HAL-PIPE-002 root-cause investigation; they do NOT block the render. If no voiced segments are identified by getSpokenSegmentNumbers, the audit is skipped entirely. Amplification is never applied. BEAT/pause/Belle/sting/music segments are excluded from this audit. The whole-stream −16 LUFS loudnorm pass on the final mix is separate and unaffected.
