...
Rule: During assembly (core.ts), every voiced segment (narrator and character dialogue, identified by getSpokenSegmentNumbers) is measured for integrated LUFS. Any segment below −28 LUFS, or any segment that cannot be measured (analyzeAudioLoudness throws), is classified as near-silent or corrupted and causes assembly to hard-fail before the mix — flagged for re-render. Amplification is never applied. BEAT/pause/Belle/sting/music segments are excluded from this audit. The whole-stream −16 LUFS loudnorm pass on the final mix is separate and unaffected by this rule.
