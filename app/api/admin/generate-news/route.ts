...
const CATEGORY_CONFIG: Record<string, { label: string; searchInstructions: string }> = {
  national: {
    label: 'National News',
    searchInstructions: 'Search for top US national news stories from today.'
  },
  international: {
    label: 'International News',
    searchInstructions: 'Search for top international and world news stories from today.'
  },
  business: {
    label: 'Business & Finance',
    searchInstructions: 'Search for top business and finance news from today.'
  },
  sports: {
    label: 'Sports',
    searchInstructions: 'Search for top sports news from today.'
  },
  science: {
    label: 'Science & Technology',
    searchInstructions: 'Search for top science and technology news from today.'
  },
  state: {
    label: 'Local News',
    searchInstructions: 'Search for local news and weather for STATE_NAME.'
  }
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (category === 'state') {
      ...
      config = {
        ...config,
        label: `${state} Local News`,
        searchInstructions: config.searchInstructions.replace('STATE_NAME', state)
      };
    }
    ...
    const currentEpisode = settingsData?.categories?.[category]?.episode_number || 0;
    const newEpisode = currentEpisode + 1;

    const updatedCategories = {
      ...settingsData?.categories,
      [category]: {
        ...settingsData?.categories?.[category],
    ...
    await supabase.from('news_settings').update({ categories: updatedCategories }).eq('id', 1);
    ...
    return NextResponse.json({
      success: true,
      episode: { category, state, episodeNumber: newEpisode, script, audioUrl, generatedAt: new Date().toISOString() }
    });
...
async function generateNewsScript(
  config: { label: string; searchInstructions: string },
  narrator: string,
  storiesCount: number
): Promise<string> {
...
- Broadcast style - conversational and clear`;
  ...
  script = script
    ...
    .replace(/<[^>]+>/g, '')
    .replace(/^.*?(I searched|I found|Let me search|Based on my|According to my|Here's what|I'll search).*?\n/gim, '')
    .replace(/\[\d+\]/g, '')
    .replace(/\(Source:.*?\)/gi, '')
    .replace(/https?:\/\/[^\s]+/g, '')
    .replace(/\n{3,}/g, '\n\n')
