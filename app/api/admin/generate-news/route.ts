...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (category === 'state' && state) {
      ...
      // Use primary source based on settings
      if (promptData.newsSourcePriority === 'newsapi' || !promptData.newsSourcePriority) {
        stories = await fetchNewsAPI(category, storyCount);
        if (stories.length < storyCount) {
          // Backup to World News API
          const backup = await fetchWorldNewsAPI(category, storyCount - stories.length);
          stories = [...stories, ...backup];
        }
      } else if (promptData.newsSourcePriority === 'worldnews') {
        stories = await fetchWorldNewsAPI(category, storyCount);
        if (stories.length < storyCount) {
          const backup = await fetchNewsAPI(category, storyCount - stories.length);
          stories = [...stories, ...backup];
        }
      } else {
        stories = await fetchGDELT(state || 'United States', storyCount);
      }
    }
