...
const SEED_AGENTS: AgentsState = {
  ...
  bart: {
    status: 'working',
    currentTask: 'BART-001 Corrected Ground Truth Financial Assessment — filed and accepted pending corrections. Awaiting Mercury balance confirmation, KIE.ai cost, and X API plan from Marc.',
    percentComplete: 75,
    waitingOn: 'Marc: new Mercury API token · Marc: KIE.ai monthly cost confirmation · Marc: X API plan (free vs $100/mo Basic)',
    lastActivity: TODAY,
    eta: 'BART-001 closes same day Marc provides 3 outstanding data points · BART-002 begins immediately after',
    whyItMatters: 'Runway is calculated from Mercury balance, which is 24 days stale. KIE.ai and X API are untracked expenses that may increase monthly burn by up to $110. Without confirmed burn rate, Bart cannot close the financial baseline and BART-002 (live framework) cannot begin.',
    lastReport: {
      text: 'BART-001 Corrected Baseline 2026-06-05: External MRR $0. Active external subscribers: 0. Stripe internal test: 1 ($59.99/yr, m.postlewaite+test2@gmail.com). Monthly burn confirmed $405.17/mo via Airtable (9 services). ElevenLabs growing_business plan: $275/mo amortized, renewal ~Feb 2027. Mercury balance last known $9,925.24 on 2026-05-12 (24 days stale — token revoked). Estimated runway 22–24 months. Two untracked services: KIE.ai (cost unknown) and X API (free or $100/mo). Break-even: 52 subscribers at $7.99/mo.',
      timestamp: new Date('2026-06-05').toISOString(),
    },
  },
...
const SEED_MISSIONS: Mission[] = [
  ...
  // ── BART ──────────────────────────────────────────────────────────────────
  {
    ...
    priority: 'P1',
    percentComplete: 75,
    waitingOn: 'Marc: new Mercury API token · Marc: KIE.ai monthly cost · Marc: X API plan (free or $100/mo)',
    lastActivity: TODAY,
  ...
  },
  {
    ...
    priority: 'P1',
    percentComplete: 0,
    waitingOn: 'Marc: accept corrected BART-001 baseline (3 data points outstanding)',
    lastActivity: TODAY,
