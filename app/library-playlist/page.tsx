...
function LibraryPlaylistContent() {
  ...
  const selectedStoryIds = playlist.map(p => p.id)
  const filteredUnselected = stories.filter(s => !selectedStoryIds.includes(s.id) && matchesFilter(s))
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0f172a', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'sticky', bottom: 0, left: 0, right: 0, backgroundColor: '#0f172a', padding: '1rem', borderTop: '1px solid #334155', zIndex: 50, flexShrink: 0 }}>
        <button onClick={startPlaylist} disabled={storyCount === 0} style={{ backgroundColor: storyCount > 0 ? '#22c55e' : '#475569', color: storyCount > 0 ? '#0f172a' : 'white', width: '100%', padding: '1rem', borderRadius: '10px', border: 'none', cursor: storyCount > 0 ? 'pointer' : 'not-allowed', fontSize: '17px', fontWeight: 'bold' }}>
