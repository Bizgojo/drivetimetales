...
function PlayerContent() {
  ...
  const handleReserve = async () => {
    ...
    try { await supabase.from('wishlists').upsert({ user_id: user.id, story_id: storyId }); router.push('/library') } catch (err) { console.error('Error reserving:', err) }
  }

  const handleNotForMe = async () => {
    if (!user || !storyId) return
    try { await supabase.from('user_passes').upsert({ user_id: user.id, story_id: storyId }); router.push('/library') } catch (err) { console.error('Error passing:', err) }
  }
...
export default function PlayerPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', backgroundColor: '#020617', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: '40px', height: '40px', border: '4px solid #f97316', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /><style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style></div>}>
      <PlayerContent />
    </Suspense>
  )
}
