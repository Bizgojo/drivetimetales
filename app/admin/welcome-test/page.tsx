...
export default function WelcomeTest() {
  ...
  async function handlePlay() {
    ...
    setPlaying(true)
    setStatus('Generating personalized welcome...')
    try {
      ...
      setStatus(data.cached ? '▶ Playing (cached)...' : '▶ Playing (freshly generated)...')
  ...
  return (
    <div style={{ fontFamily: 'Georgia, serif', background: '#FAF9F6', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 48, width: 480, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
        ...
        <p style={{ color: '#666', fontSize: 14, marginBottom: 32 }}>Name baked in — one seamless file, no stitching. Cached after first generation.</p>
        ...
        <div style={{ marginTop: 24, padding: 16, background: '#f8f8f8', borderRadius: 8, fontSize: 12, color: '#666', lineHeight: 1.6 }}>
          <div style={{ fontWeight: 600, marginBottom: 4, color: '#444' }}>Script:</div>
          <div>"Hey <strong style={{ color: '#f97316' }}>{name || '[name]'}</strong>! I'm Belle. I'll be here before every story. Just a friend who knows what's worth your time. You're going to love this one."</div>
        </div>
      </div>
    </div>
  )
