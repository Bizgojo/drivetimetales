...
function MyLibraryContent() {
  ...
  if (error) {
    return <div className="py-12 text-center text-red-400">Error: {error.message}</div>
  }
  ...
  const myStories = myStoryIds.map(id => {
    const story = storyLookup[id]
    if (!story) return null
    return { ...story, progress: listeningHistory[id].progress }
  }).filter(Boolean)
  const wishlistStories = wishlistIds.map(id => storyLookup[id]).filter(Boolean)
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="px-4 py-6">
        ...
        <div className="flex gap-2 mb-6">
          <button onClick={() => setTab('library')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'library' ? 'bg-orange-500 text-black' : 'bg-slate-800 text-white'}`}>📚 My Stories</button>
          <button onClick={() => setTab('wishlist')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'wishlist' ? 'bg-orange-50
cd /Users/williampostlewaite/Downloads/drivetimetales/temp-repo
cat > app/my-library/page.tsx << 'EOF'
'use client'
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="px-4 py-6">
        ...
        <div className="flex gap-2 mb-6">
          <button onClick={() => setTab('library')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'library' ? 'bg-orange-500 text-black' : 'bg-slate-800 text-white'}`}>📚 My Stories</button>
          <button onClick={() => setTab('wishlist')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'wishlist' ? 'bg-orange-500 text-black' : 'bg-slate-800 text-white'}`}>❤️ Wishlist</button>
        </div>
        {tab === 'library' && (
          <div className="text-center py-12">
