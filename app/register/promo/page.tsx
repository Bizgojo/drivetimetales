...
function PromoRegisterContent() {
  ...
  useEffect(() => {
    // Get promo subscription data from localStorage
    const storedPromo = localStorage.getItem('dtt_promo_subscription')
    if (!storedPromo) {
      // No promo code redeemed, redirect to welcome
      router.push('/welcome')
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create user profile with promo subscription
      const { error: profileError } = await supabase
        ...
        .insert({
          ...
          email: email.trim(),
          display_name: name.trim(),
          subscription_type: promoData.type || 'promo_trial',
          subscription_start: promoData.startDate,
          subscription_end: promoData.endDate,
          promo_code_used: promoData.code,
          credits: 999, // Unlimited during promo period
      ...
      // Update promo code with user info
      await supabase
        .from('promo_codes')
        .update({ 
          redeemed_by_email: email.trim(),
          redeemed_by_name: name.trim()
        })
        .eq('code', promoData.code)
      
      // Clear the temporary promo data
      localStorage.removeItem('dtt_promo_subscription')
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-md mx-auto px-4 py-8">
        ...
        <div className="bg-green-500/20 border border-green-500/50 rounded-xl p-4 mb-6 text-center">
          <p className="text-green-400 text-lg font-bold mb-1">🎉 Promo Code Accepted!</p>
          <p className="text-green-300 text-sm">
            You have <span className="font-bold">{promoData.days} days</span> of free access
          </p>
        ...
        <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {isSubmitting ? 'Creating Account...' : 'Start Listening Free'}
            </button>
          </form>
        ...
        <p className="text-slate-400 text-sm text-center mt-6">
          ...
          <a href="/signin" className="text-orange-400 hover:underline font-medium">Sign In</a>
        </p>
      </div>
    </div>
  )
...
export default function PromoRegisterPage() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <PromoRegisterContent />
    </Suspense>
  )
}
