...
**Status:** CANON — declared by Marc Postlewaite, Aug 8, 2026. Rules 1–15 are binding on every story from this point.  
...
| 6 | Pipeline | Wire `resolveVoiceSegment()` into `generate-voices` route | ✅ ATL-SFX-WIRE-001 (main, 2026-07-29) |
| 7 | Pipeline | Wire `validateManifestGate()` into `render-final-mix` route | ✅ ATL-SFX-WIRE-001 (main, 2026-07-29) |
| 8 | Backfill | `scripts/sfx-lock-backfill.js` — PV1+PV2 (54 segments) | ✅ run complete |
| 9 | Rule 8 ext | Extend `validateManifestGate()` to assert `background_music.mp3` present before mix | ✅ ATL-RULE8-EXT-001 (this PR) |
...
*Promoted from `governance/drafts/SFX-ASSET-LOCK-001.md` and declared canon by Marc Postlewaite, Aug 8, 2026.*
