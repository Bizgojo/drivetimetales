...
async function main() {
  ...
  // Update stories
  const { data: updateData, error: updateErr, count: updateCount } = await supabase
    .from('stories')
    .update({ voice_profile_id: '88b97709-c5af-46ee-84a3-7c5209808231' })
    .eq('author_id', 'db62ba17-be3c-4f4e-a105-90449ef84a25')
    .select('id');
  
  if (updateErr) {
    console.error('UPDATE error:', JSON.stringify(updateErr));
  } else {
    console.log('UPDATE success, rows linked:', updateData?.length);
  }
}
