...
export default function AuthorsNarratorsPage() {
  ...
  async function addAuthor() {
    ...
    const { error } = await supabase.from('authors').insert({
      ...
      style_reference: newAuthor.style_reference || null,
      style_description: newAuthor.style_description || null,
      sort_order: newAuthor.sort_order ? Number(newAuthor.sort_order) : null,
      is_active: newAuthor.is_active !== false,
    ...
    setNewAuthor({ name: '', primary_genre: '', secondary_genre: '', narrative_voice: 'third_limited', tone: '', pacing: 'medium', signature: '', example_line: '', style_reference: '', style_description: '', sort_order: '', is_active: true, narrator_voice_id: '' })
    load()
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 900, color: text }}>Authors & Narrators</h1>
              <div style={{background:"#bfdbfe",color:"#111827",fontWeight:800,padding:"6px 10px",borderRadius:8,marginBottom:12,display:"inline-block"}}>AUTHORS PAGE BUILD MARKER 2026-04-21 09:35 AM</div>
            ...
            {showAddAuthor && (
              <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <input style={inp} placeholder="Real author style reference (e.g. Stephen King)" value={newAuthor.style_reference} onChange={e => setNewAuthor(p => ({ ...p, style_reference: e.target.value }))} />
                  <textarea style={{ ...inp, height: 70, resize: 'vertical' }} placeholder="Style description" value={newAuthor.style_description} onChange={e => setNewAuthor(p => ({ ...p, style_description: e.target.value }))} />
                  <input style={inp} placeholder="Sort order within genre (1, 2, 3)" value={newAuthor.sort_order} onChange={e => setNewAuthor(p => ({ ...p, sort_order: e.target.value }))} />
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#111827', fontSize: 13, fontWeight: 600 }}>
                    <input type="checkbox" checked={newAuthor.is_active !== false} onChange={e => setNewAuthor(p => ({ ...p, is_active: e.target.checked }))} />
                    Active in V2
                  </label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {authors.map(a => (
                <div key={a.id} onClick={() => setSelectedAuthor(a)} style={{ background: selectedAuthor?.id === a.id ? '#fff8f3' : card, border: `2px solid ${selectedAuthor?.id === a.id ? orange : border}`, borderRadius: 12, padding: '12px 14px', cursor: 'pointer' }}>
                  ...
                  {(a.style_reference || a.style_description) && (
                    <div style={{ color: '#4b5563', fontSize: 12, marginTop: 4 }}>
                      ✍️ {a.style_reference || 'Style reference not set'}
                    </div>
                  )}
            ...
            {selectedAuthor ? (
              <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 16, padding: 28 }}>
                ...
                <div style={{ borderTop: `1px solid ${border}`, paddingTop: 20, marginBottom: 20 }}>
                  <div style={{ fontWeight: 800, color: text, fontSize: 15, marginBottom: 12 }}>✍️ Style Reference</div>
                  <div style={{ background: '#f9fafb', borderRadius: 10, padding: '12px 14px', marginBottom: 10 }}>
                    <div style={{ color: muted, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Real author style</div>
                    <div style={{ color: text, fontWeight: 700, fontSize: 14 }}>{selectedAuthor.style_reference || 'Not set'}</div>
                  </div>
                  <div style={{ background: '#f9fafb', borderRadius: 10, padding: '12px 14px' }}>
                    <div style={{ color: muted, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Style description</div>
                    <div style={{ color: text, fontSize: 14, lineHeight: 1.6 }}>{selectedAuthor.style_description || 'No style description yet.'}</div>
                  </div>
                </div>
