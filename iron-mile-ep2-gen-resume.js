// Iron Mile EP2 — resume after micro-split, seg 3–99
// Pre-approved inline ASR: nineteen-eighties → 1980s if seg 5 fails.
...
function tryInlineAsrPatch(expected) {
  ...
  let candidate = expected
    ...
    // Decade forms (pre-approved for this episode)
    .replace(/\bnineteen-eighties\b/gi, '1980s')
    .replace(/\btwenty-nineteen\b/gi, '2019')
    // Hyphen stripping in compound adjectives Whisper drops
...
async function applyScriptPatch(oldText, newText) {
  ...
  if (count !== 1) throw new Error(`Patch ABORT: expected 1 occurrence, found ${count}`);
...
async function main() {
  console.log(`[${new Date().toISOString()}] EP2 seg ${START_SEG}–${END_SEG} (99 total after micro-split)`);
  let done = 0;
  ...
  for (let seg = START_SEG; seg <= END_SEG; seg++) {
    ...
    if (failures.length > 0) {
      ...
      // Auto-patch approved ASR classes (QC_MISMATCH only — not truncation)
        ...
        if (patch) {
          ...
          try {
            ...
            if (rf.length === 0 && (retry.body.generatedSegments || []).length > 0) {
              ...
              console.error(`  → RETRY FAILED: ${rf[0]?.error?.substring(0,80) || 'unknown'}`);
